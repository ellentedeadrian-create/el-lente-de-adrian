...
exports.handler = async function (event) {
  ...
  try {
    ...
    const { sourceId, amount, amountTotal, nombre, contacto, tipoSesion, paquete, fecha, ciudad } = body;
    ...
    const totalTexto = amountTotal ? ` (paquete total: $${(amountTotal/100).toFixed(2)}, saldo pendiente: $${((amountTotal-amount)/100).toFixed(2)})` : '';

    const response = await paymentsApi.createPayment({
      sourceId: sourceId, // "ficha" segura de la tarjeta, generada en el navegador por el SDK de Square
      idempotencyKey: crypto.randomUUID(),
      amountMoney: {
        amount: amount, // en centavos. Depósito del 50% del paquete.
        currency: 'USD'
      },
      locationId: process.env.SQUARE_LOCATION_ID,
      note: `Depósito 50% - ${tipoSesion} - ${paquete} - ${fecha} - ${nombre} (${contacto}) - ${ciudad}${totalTexto}`
    });
